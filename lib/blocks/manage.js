...
({
	...
	getParent: function()
	{
		...
		{
			parent_level = this.getLevel(parent)
	...
	getDirectChildren: function()
	{
		...
		{
			child_level = this.getLevel(child)
	...
	setLevel: function(level)
	{
		...
		, diff = level - indentations.length
...
window.addEvent('domready', function() {
	...
	document.body.addEvent('click:relay(#manager td.cell--is-navigation-excluded i)', function(ev, el) {
...
!function() {
	...
	document.body.addEvent('mousedown:relay(.listview .handle)', function(ev, el) {

		if (initialOrder === null)
		{
			initialOrder = el.getParent('tbody').getElements('tr')

			console.info('initiate initian order:', initialOrder)
		}
	 ...
	 */
	document.body.addEvent('click:relay([data-actionbar-context="update-tree"] [data-action="cancel"])', function(ev, el) {

		var body = document.body.getElement('.listview tbody')
	 ...
	 */
	document.body.addEvent('click:relay([data-actionbar-context="update-tree"] [data-action="save"])', function(ev, trigger) {

		var order = [],
		relation = {}
		, body = document.body.getElement('.listview tbody')
